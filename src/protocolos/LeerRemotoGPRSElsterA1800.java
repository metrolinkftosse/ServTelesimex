 ...
 * @author dperez
 */
public class LeerRemotoGPRSElsterA1800 implements Runnable, SerialPortEventListener {
...
//    public LeerRemotoGPRSElsterA1800(EMedidor med, Vector<EConfModem> confmodem, boolean perfil, boolean eventos, boolean registros, AppletLecturaMedidores lm, int indx, ControlProcesos cp, Abortar objabortar, boolean aviso) {
    public LeerRemotoGPRSElsterA1800(EMedidor med, Vector<EConfModem> confmodem, boolean perfil, boolean eventos, boolean registros, int indx, ControlProcesos cp, Abortar objabortar, boolean aviso, ZoneId zid) {
        this.med = med;
    ...
    private void interpretaCadena(String cadenahex) {
        ...
        if (conexionModem) {
            if (cadenahex.contains("4E 4F 20 43 41 52 52 49 45 52") && !desbloqueo) {
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion portadora no encontrada");
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion sin linea");
                try {
                    if (aviso) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion linea ocpuada");
                try {
                    if (aviso) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion modem no responde");
                ...
                if (numeroReintentos >= actualReintento) {
                    if (!objabortar.labortar) {
                        ...
                        escribir("Proceso Abortado");
                        cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion proceso abortado");
                        cerrarLog("Abortado", false);
                        leer = false;
                    }
                } else {
                    //terminahilos();
                    cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion numero de reintentos agotado");
                    cerrarLog("Desconexion Numero de reintentos agotado", false);
                    escribir("Estado de lectura no leido");
                    leer = false;
                }
            ...
            if (lcomandoI) {
                revisaComandoI(vectorhex, cadenahex);
            } else if (lidentidad) {
    ...
    private void abrePuerto() {
        ...
        try {
            if (!objabortar.labortar) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Conexion satisftactoria");
            } else {
            ...
            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Error al conectar con el modem");
            if (portconect) {
                cerrarPuerto();
            }
            //e.printStackTrace();
    ...
    private void iniciacomunicacion() {
        ...
        if (portconect) {
            ...
            if (med.isLconfigurado()) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion medidor no configurado");
                cerrarPuerto();
                try {
                    if (aviso) {
                ...
                try {
                    ...
                    while (t) {
                        ...
                        if (intentosRetransmision > 3) {
                            ...
                            int intentosalir = 0;
                            while (salir) {
                                if (!enviando || intentosalir > ((tiemporetransmision / 500)) * 2) {
                    ...
                    if (cierrapuerto) {
                        ...
                        if (numeroReintentos >= actualReintento) {
                            ...
                            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion numeroe reintentos agotado");
                    ...
                    if (cierrapuerto) {
                        ...
                        if (numeroReintentos >= actualReintento) {
                            if (!objabortar.labortar) {
                                ...
                                escribir("Proceso Abortado");
                                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "proceso abortado");
                                cerrarLog("Abortado", false);
                                leer = false;
                            }
                        } else {
                            //terminahilos();
                            escribir("Numero de reintentos agotado");
                            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconectado numero de reintentos agotado");
                            escribir("Estado Lectura No Leido");
                            cerrarLog("Desconexion Numero de reintentos agotado", false);
                            leer = false;
                        }
                ...
                try {
                    ...
                    if (cierrapuerto) {
                        ...
                        if (numeroReintentos >= actualReintento) {
                            ...
                            //terminahilos();
                            escribir("Numero de reintentos agotado");
                            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconectado numero de reintentos agotadohhhhhhhhhhhh");
                            escribir("Estado Lectura No Leido");
                            cerrarLog("Desconexion Numero de reintentos agotado", false);
                            leer = false;
                        }
...
//        }
    ...
    public int obtenerOffset() {
        int offset = 0;
        offset = (ndias * (24 / ((NBR_BLK_INTS_SET1 * intervalos) / 60))) * (ElementCount);
        return offset;
    }

    private void revisaComandoI(String[] vectorhex, String cadenahex) {
        if (vectorhex.length >= 1) {
            ...
            if (cadenahex.contains("50 53 45")) {
                ...
                enviaTrama2(tramanueva, "");
            } else {
                inicia1 = false;
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion Error de protocolo");
                cerrarPuerto();
    ...
    private void revisarIdentidad(String[] vectorhex) {
        if (seEspera06) { //si se espera un 06
            ...
            if (vectorhex[0].equals("EE")) { //esperamos una retransmision o un complemento
                ...
                if (siguienteTrama) {//es complemento de una trama
                    ...
                    if ((vectorhex.length - 8) >= (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16))) { //validamos campo len
                        if (validacionCRCFCS2(vectorhex, (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16)) + 8)) {//validamos crc para
                            if (((((byte) Integer.parseInt(vectorhex[2], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {
                                if (vectorhex[10].equals("02")) {
                                    //lleva ticket
                                    isticket = true;
                                } else {
    ...
    private String[] EliminarRuido(String[] vectorhex) {
    ...
    private String[] EliminarRuido2(String[] vectorhex) {
        String nuevoVector[] = vectorhex;
        int indiceInicio = 0;
        boolean entro = true;
            ...
            if (vectorhex[0].equals("EE")) {// Es una retransmision o siguiente bloque
                ...
                if (complemento) {
                    ...
                    if (vectorhextemp[0].equals("06")) { // complemento de 06
                        ...
                        if (vectorhex.length > 6) {//pose cabecera?
                            if ((vectorhex.length - 8) >= (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16))) { //validamos campo len
                                if (validacionCRCFCS(vectorhex, (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16)) + 8)) {//validamos CRC
                                    if (((((byte) Integer.parseInt(vectorhextemp[2], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                        if (isticket) {
                                            //hay autenticacion
                                            byte trama[] = tramasElster.getAutenticacion();
                        ...
                        if (validacionCRCFCS2(vectorhex, (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16)) + 8)) {//validamos CRC
                            if (((((byte) Integer.parseInt(vectorhex[2], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                if (vectorhex[6].equals("00") || vectorhex[6].equals("0A")) { //ack del data
                                    ...
                                    seEspera06 = true;
                                    tiempo = 4000;
                                    //System.out.println("Envia logon");
                                    escribir("Envia Logon");
                                    try {
                                        avisoStr("Logon");
                                    } catch (Exception e) {
                ...
                if (vectorhex.length > 6) {//tiene cabecera
                    if ((vectorhex.length - 8) >= (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16))) {//validamos el tamaño es mayor o = al campo len
                        if (validacionCRCFCS2(vectorhex, (Integer.parseInt((vectorhex[4] + vectorhex[5]), 16)) + 8)) {
                            if (((((byte) Integer.parseInt(vectorhex[2], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                escribir("Numero de secuencia " + vectorhex[3]);
                        ...
                        if (vectorhextemp.length > 6) {//tiene cabecera
                            if ((vectorhextemp.length - 8) >= (Integer.parseInt((vectorhextemp[4] + vectorhextemp[5]), 16))) {//validamos el tamaño es mayor o = al campo len
                                if (validacionCRCFCS2(vectorhextemp, (Integer.parseInt((vectorhextemp[4] + vectorhextemp[5]), 16)) + 8)) {
                                    if (((((byte) Integer.parseInt(vectorhex[2], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                        escribir("Numero de secuencia " + vectorhextemp[3]);
                                ...
                                if (validacionCRCFCS(vectorhextemp, (Integer.parseInt((vectorhextemp[5] + vectorhextemp[6]), 16)) + 9)) {
                                    if (((((byte) Integer.parseInt(vectorhextemp[3], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                        ...
                                        if (vectorhextemp[7].equals("00")) {
                                            escribir("Numero de secuencia " + vectorhextemp[4]);
                            ...
                            if (((((byte) Integer.parseInt(vectorhex[3], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                if (vectorhex[7].equals("00")) { //ack del data
                                    ...
                                    if (continuar) {
                                        ...
                                        cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion error en serial");
                                        cerrarPuerto();
...
////                                    System.out.println("Serial" + val);
                                            ...
                                            if (continuar) {
                                                ...
                                                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSElster", "Desconexion serial incorrecto");
                                                cerrarPuerto();
                        ...
                        if (validacionCRCFCS(vectorhex, (Integer.parseInt((vectorhex[5] + vectorhex[6]), 16)) + 9)) {//validacion crc
                            if (((((byte) Integer.parseInt(vectorhex[3], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {// bit spoling esperado
                                ...
                                if (primerbloque) {
                                    ...
                                    escribir("numero de secuencia inicial " + contadorMT017);
                                } else {
                                    escribir("Numero de secuencia " + vectorhex[4]);
                                }
    ...
    private void revisarlogoff(String[] vectorhex) {
        if (vectorhex[0].equals("06")) {// se espera 06
            if (vectorhex.length > 7) {//pose cabecera?
                if ((vectorhex.length - 9) >= (Integer.parseInt((vectorhex[5] + vectorhex[6]), 16))) { //validamos campo len
                    if (validacionCRCFCS(vectorhex, (Integer.parseInt((vectorhex[5] + vectorhex[6]), 16)) + 9)) {//validamos CRC
                        if (((((byte) Integer.parseInt(vectorhex[3], 16) & 0xFF) & 0x20) >> 5) == (bitSpoling >> 5)) {//
                            escribir("numero de secuencia " + vectorhex[4]);
                            cerrarPuerto();
