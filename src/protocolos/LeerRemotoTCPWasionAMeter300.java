 ...
 * @author jgarcia
 */
public class LeerRemotoTCPWasionAMeter300 {
    //variables constructor
    private final String name = "Wasion_aMeter300";
    ...
    int InvokeIDandParity = 65; //0xC1 := 193
    int indxLength = 2;
    int users[] = {1}; //dirección Cliente HLS: 1
    ...
    private void init() throws InterruptedException {
        ...
        numBytesDir = Integer.parseInt((med.getBytesdireccion().equals("5") ? "4" : med.getBytesdireccion())); 
        ...
        epp.setIncreaseCounter_HLS(false);
        epp.editDLMS_PrincipalOBIS(4, "00002B0100FF"); // Invocation Counter
        ...
        DLMS.setSC_HLS( (byte) 0x10);//Security Control Byte
        if (!ps.leer) {
            ps.escribir("Inicialización de Procesos DLMS fallida");
            return;
        }
    ...
    public void setAARQFields(int nSession){
        ...
        byte[] ApTitle = {(byte) 0x57, (byte) 0x53, (byte) 0x45, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x01};//Podría parametrizarse
        byte[] versionDLMS = {(byte) 0x06};
        byte[] conformance = {(byte) 0xFF, (byte) 0xFF, (byte) 0xFF};
        byte[] pduSize = {(byte) 0x04, (byte) 0xFD}; 
        if (nSession == 1){
    ...
    public ElecturaAux procesaLec(Timestamp fprimerintervalo, Timestamp fechaintervalo, Timestamp fechaCero, int intervalo, Electura lec, ElecturaAux elecaux){
        //System.out.println("Procesa valor Wasion");
        try{
    ...
    public ElecturaAux procesaReg(Timestamp fprimerintervalo, Timestamp fechaintervalo, Timestamp fechaCero, int intervalo, ERegistro reg, ElecturaAux elecaux){
        //System.out.println("Registros - Procesa valor Wasion");
        try{
        int indx = elecaux.getIndexlec();
