 ...
 * @author dperez
 */
public class LeerRemotoGPRSElgama implements Runnable, SerialPortEventListener {
    ...
    TramasRemotaPSTNElgama tramasgprs = new TramasRemotaPSTNElgama();
    String cadenahex = "";
    long timeout;
    boolean lperfil;
    boolean leventos;
...
//    public LeerRemotoGPRSElgama(EMedidor med, Vector<EConfModem> confmodem, boolean perfil, boolean eventos, boolean registros, AppletLecturaMedidores lm, int indx, ControlProcesos cp, Abortar objabortar, boolean aviso) {
    public LeerRemotoGPRSElgama(EMedidor med, Vector<EConfModem> confmodem, boolean perfil, boolean eventos, boolean registros, int indx, ControlProcesos cp, Abortar objabortar, boolean aviso, ZoneId zid) {
        this.med = med;
...
//            file = new File(cp.rutalogs + "" + this.med.getMarcaMedidor().getNombre() + "/GPRS_" + med.getnSerie() + ".txt");
    ...
    private void jinit() {
        ...
        if (confModem.size() > 0) {
            ...
            if (portconect) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Intento de conexion sin respuesta");
            ...
            try {
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), med.getnSerie(), "LeerGPRSELgama", "Intento de conexion sin configuracion de modem");
    ...
    private void interpretaCadena(String cadenahex) {
        ...
        if (conexionModem) {
            if (cadenahex.contains("4E 4F 20 43 41 52 52 49 45 52") && !desbloqueo) {
                ...
                escribir("<= " + cadenahex);
                escribir("No carrier");
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion No se encuentra portadora");
                try {
                    if (aviso) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion No se encuentra linea");
                try {
                    if (aviso) {
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion portadora ocupada");
                try {
                    if (aviso) {
                ...
                tiemporetransmision = timeout * 5000;
                if (aviso) {
                    try {
                ...
                byte[] tramaelgama = tramasgprs.getLocal0();
                ...
                if (trama.equals("")) {
                    ...
                    if (med.getTipoconexion() == 1) {
                        vtrama = tramasgprs.StringToHexMarcadoPSTN(med.getNumtelefonico().trim());
                    } else {
                        vtrama = tramasgprs.StringToHexMarcadoGPRS(med.getDireccionip().trim() + "/" + med.getPuertoip().trim());
                    }
                    esperandoconexion = true;
                    enviaTramaConexion(vtrama, "=> Solicitud de conexion IP/puerto " + med.getDireccionip().trim() + "/" + med.getPuertoip().trim());
                ...
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion Sin respuesta");
            ...
            if (cadenahex.contains("4E 4F 20 43 41 52 52 49 45 52")) {//Se cayo el modem o se desconecto               
                ...
                if (!complemento) {//no es complemento
                    if (vectorhex[0].equals("2F")) { // llega con 2F
                            ...
                            if (vectorhex.length - 1 >= Integer.parseInt(vectorhex[1], 16)) {
                                if (adp == Integer.parseInt(vectorhex[8], 16)) {//es el adp correcto
                                    if (validaCRCElgama(vectorhex, Integer.parseInt(vectorhex[1], 16) + 1)) {
                                        ...
                                        adp += 1;
                                        adp = adp % 256;
                                        byte tramaelgama[] = tramasgprs.getLocal1();
                                        tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                        tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                        tramaelgama[4] = (byte) (Integer.parseInt(vecSerieMedidor[2], 16) & 0xFF);
                                        tramaelgama[5] = (byte) (Integer.parseInt(vecSerieMedidor[3], 16) & 0xFF);
                                        ...
                                        escribir("BAD CRC");
                                        escribir("=> Espera a envio de ultima trama " + tramasgprs.encode(ultimatramaEnviada, ultimatramaEnviada.length));
                                    }
                        ...
                        if (vectorhex.length > 7) {
                            //el primer dato es el tamaño del vector
                            if (vectorhex.length >= Integer.parseInt(vectorhex[0], 16) && !vectorhex[0].equals("00")) {
                                if (adp == Integer.parseInt(vectorhex[7], 16)) {//es el adp correcto
                                    if (validaCRCElgama2(vectorhex, Integer.parseInt(vectorhex[0], 16))) { //vañoda tamaño crc sin 2F
                                        actualUltimatrama = 0;
                            ...
                            escribir("Trama basura " + vectorhex.length + " bytes");
                            escribir("=> Espera a envio de ultima trama " + tramasgprs.encode(ultimatramaEnviada, ultimatramaEnviada.length));
                            enviaTramaUltima(ultimatramaEnviada, "Solicitud de congiguracion");
                        }
                    }
                    ...
                    } else {//puede llegar sin el 2F
                        if (vectorhextemp.length > 7) {//trae el tamaño o no ?
                            if (vectorhextemp.length >= Integer.parseInt(vectorhextemp[0], 16) && !vectorhextemp[0].equals("00")) {//valida tamaño sin 2F
                                    ...
                                    if (validaCRCElgama2(vectorhextemp, Integer.parseInt(vectorhextemp[0], 16))) {
                                        ...
                                        if (Integer.parseInt(vectorhextemp[0], 16) > 19) { //es version nueva
                                            if (Integer.parseInt(vectorhextemp[17], 16) < 5) {
                                                ...
                                                exponent = Integer.parseInt(vectorhextemp[22], 16);
                                                escribir("KTRU: " + ktru);
                                                escribir("KTRI: " + ktri);
                                                escribir("EXP: " + exponent);
                                                //System.out.println("KTRU: " + ktru);
                            ...
                            if (vectorhex.length - 1 >= Integer.parseInt(vectorhex[1], 16)) {
                                if (adp == Integer.parseInt(vectorhex[8], 16)) {//es el adp correcto
                                    if (validaCRCElgama(vectorhex, Integer.parseInt(vectorhex[1], 16) + 1)) {
                                        ...
                                        inicio2 = false;
                                        if (leventos) {
                                            tramaelgama = tramasgprs.getLocal4();
                                            tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                            tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                            tramaelgama[4] = (byte) (Integer.parseInt(vecSerieMedidor[2], 16) & 0xFF);
                                            tramaelgama[5] = (byte) (Integer.parseInt(vecSerieMedidor[3], 16) & 0xFF);
                                            ...
                                            if (lregistros) {
                                                ...
                                                inicio3 = true;
                                                tramaelgama = tramasgprs.getLocal1();
                                                tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                                tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                                tramaelgama[4] = (byte) (Integer.parseInt(vecSerieMedidor[2], 16) & 0xFF);
                                                tramaelgama[5] = (byte) (Integer.parseInt(vecSerieMedidor[3], 16) & 0xFF);
                                                ...
                                                tramaelgama = tramasgprs.getLocal2();
                                                tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                                tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                                tramaelgama[4] = (byte) (Integer.parseInt(vecSerieMedidor[2], 16) & 0xFF);
                                                tramaelgama[5] = (byte) (Integer.parseInt(vecSerieMedidor[3], 16) & 0xFF);
                                                ...
                                                mensaje = "=> Solicitud de Perfil de carga 1 parte del canal " + ncanal + " del dia " + ndia;
                                                enviaTrama2(tramaelgama, mensaje);
                                            } else {
                                                cerrarPuerto();
                        ...
                        if (vectorhex.length > 8) {
                            if (vectorhex.length - 1 >= Integer.parseInt(vectorhex[1], 16)) {
                                if (adp == Integer.parseInt(vectorhex[8], 16)) {//es el adp correcto
                                    if (validaCRCElgama(vectorhex, Integer.parseInt(vectorhex[1], 16) + 1)) {
                                        actualUltimatrama = 0;
                                        String eventos1[] = new String[Integer.parseInt(vectorhex[1], 16) + 1];
                                        ...
                                        byte tramaelgama[] = tramasgprs.getLocal5();
                                        tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                        tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                        tramaelgama[4] = (byte) (Integer.parseInt(vecSerieMedidor[2], 16) & 0xFF);
                                        tramaelgama[5] = (byte) (Integer.parseInt(vecSerieMedidor[3], 16) & 0xFF);
                                        ...
                                        enviaTramaUltima(ultimatramaEnviada, "=>Solicitud de eventos de falla en energia");
                                    }
                                } else {//adp incorrecto
                                    escribir("ADP Incorrecto");
                                    escribir("=> Espera a envio de ultima trama " + tramasgprs.encode(ultimatramaEnviada, ultimatramaEnviada.length));
                ...
                if (!complemento) {
                    if (vectorhex[0].equals("2F")) {//llego correctamente
                        ...
                        if (vectorhex.length > 7) {
                            if (vectorhex.length >= Integer.parseInt(vectorhex[0], 16) && !vectorhex[0].equals("00")) {//valida tamaño sin 2F
                                if (adp == Integer.parseInt(vectorhex[7], 16)) {//es el adp correcto
                                    if (validaCRCElgama2(vectorhex, Integer.parseInt(vectorhex[0], 16))) {
                                        actualUltimatrama = 0;
                                        String[] trama = new String[(Integer.parseInt(vectorhex[0], 16))];
                ...
                if (!complemento) {
                    if (vectorhex[0].equals("2F")) {
                        if (vectorhex.length > 8) {//tiene cabecera
                            if (vectorhex.length - 1 >= Integer.parseInt(vectorhex[1], 16)) {
                                if (adp == Integer.parseInt(vectorhex[8], 16)) {//es el adp correcto
                                    if (validaCRCElgama(vectorhex, Integer.parseInt(vectorhex[1], 16) + 1)) {
                                        actualUltimatrama = 0;
                                        ...
                                        if (ndiaReg >= 0) {//continuamos solicitando
                                            byte tramaelgama[] = tramasgprs.getLocal6();
                                            adp += 1;
                                            adp = adp % 256;
                                            tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                            tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                                            ...
                                            byte tramaelgama[] = tramasgprs.getLocal7();
                                            adp += 1;
                                            adp = adp % 256;
                                            tramaelgama[2] = (byte) (Integer.parseInt(vecSerieMedidor[0], 16) & 0xFF);
                                            tramaelgama[3] = (byte) (Integer.parseInt(vecSerieMedidor[1], 16) & 0xFF);
                    ...
                    String vectorhextemp[] = tramaIncompleta.split(" ");
                    if (vectorhextemp[0].equals("2F")) {
                        if (vectorhextemp.length > 8) {//tiene cabecera
                            if (vectorhextemp.length - 1 >= Integer.parseInt(vectorhextemp[1], 16)) {
                                if (adp == Integer.parseInt(vectorhextemp[8], 16)) {//es el adp correcto
                                    if (validaCRCElgama(vectorhextemp, Integer.parseInt(vectorhextemp[1], 16) + 1)) {
                                        actualUltimatrama = 0;
                                    ...
                                    if (validaCRCElgama(vectorhex, Integer.parseInt(vectorhex[1], 16) + 1)) {
                                        ...
                                        if (nmesReg >= 0) {
                                            ...
                                            if (lperfil) {
                                                ...
                                                ndia = ndias - 1;
                                                ncanal = 1;
                                                byte tramaelgama[] = tramasgprs.getLocal2();
                                                String mensaje = "";
                                                lperfilCarga = true;
                                                parte = 0;
                                                loadProfile1 = new Vector<String[]>();
    ...
    private void abrePuerto() {
        ...
        try {
            ...
            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Conexion Satisfactoria");
        } catch (Exception e) {
            escribir(e.getMessage());
            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Error al conectar con el modem");
            if (portconect) {
                cerrarPuerto();
            }
            if (aviso) {
        ...
        if (portconect) {
            ...
            if (med.isLconfigurado()) {
                ...
                if (med.getTipoconexion() == 0) {
                    ...
                    cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Error tipo de conexion debe ser GPRS");
                    cerrarPuerto();                    
                    escribir("Estado Lectura No Leido");
                    cerrarLog("Error de configuracion", false);
                    leer = false;
                }
            } else {
                //si no esta configurado cerramos el puerto
                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Error Medidor sin configuracion ");
                cerrarPuerto();                
                escribir("Estado Lectura No Leido");
                cerrarLog("Medidor configurado", false);
                leer = false;
            }
        } else {
            cerrarLog("Medidor no configurado", false);
            ...
            public void run() {
                try {
                    ...
                    if (!reinicia && i > 119) {
                        ...
                        cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion Tiempo de espera para conexion agotado");
                        if (serialPort != null) {
                            cerrarPuerto();
                            if (numeroReintentos >= actualReintento) {
                                if (!objabortar.labortar) {
                                ...
                                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion Numero de reintentos agotado");                                
                                escribir("Estado Lectura No Leido");
                                cerrarLog("Desconexion Numero de reintentos agotado", false);
                                leer = false;
                            }
                    ...
                    if (cierrapuerto) {
                        ...
                        if (numeroReintentos >= actualReintento) {
                            if (!objabortar.labortar) {
                                ...
                                cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion Aborto de operacion");
                                escribir("Proceso Abortado");
                                cerrarLog("Abortado", false);
                                leer = false;
                            }
                            ...
                            //terminahilos();
                            cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion Numero de envios agotado");
                            escribir("Numero de reintentos agotado");                            
                            escribir("Estado Lectura No Leido");
                            cerrarLog("Desconexion Numero de reintentos agotado", false);
                            leer = false;
    ...
    private int CRCElgama(byte[] tramaelgama) { //este se eejcuta cuando hay 2F
        int[] table = {
    ...
    private boolean validaCRCElgama(String[] vectorhex, int len) { //metodo para cuando llega con 2F
        boolean ok = false;
        try {
        ...
        if (actualUltimatrama > reintentosUltimatrama) {
            ...
            if (numeroReintentos >= actualReintento) {
                if (!objabortar.labortar) {
                    ...
                    cp.escribirLog(usuario, new Timestamp(Date.from(ZonedDateTime.now(zid).toInstant()).getTime()), seriemedidor, "LeerGPRSELgama", "Desconexion proceso abortado");
                    escribir("Proceso Abortado");
                    cerrarLog("Abortado", false);
                    leer = false;
                }
